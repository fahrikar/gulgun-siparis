/* functions/index.js — Gülgün sipariş bildirimi (HTTP)
   hedef: 'admin'  → Günkar (yeni sipariş)
          'musteri'→ Gülgün (sipariş durumu değişti) */
...
functions.http('bildirimGonder', async (req, res) => {
  ...
  try {
    ...
    const baslik = govde.baslik || 'Günkar Ambalaj';
    const metin  = govde.metin  || '';
    const hedef  = govde.hedef === 'musteri' ? 'musteri' : 'admin';

    const snap = await getFirestore().collection('push_tokens').get();

    // Rol alanı olmayan eski kayıtlar admin sayılır
    const tokens = snap.docs
      .filter(d => {
        const rol = d.data().rol || 'admin';
        return rol === hedef;
      })
      .map(d => d.data().token)
      .filter(Boolean);
    ...
    if (!tokens.length) {
      res.json({ ok: true, gonderilen: 0, hedef, not: 'bu rolde kayıtlı cihaz yok' });
      return;
    }
    ...
    res.json({ ok: true, hedef, gonderilen: sonuc.successCount, toplam: tokens.length });
  } catch (e) {
    console.error(e);
